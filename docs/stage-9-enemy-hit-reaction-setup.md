...
3. Drag its **RigidBody2D** into **Rigid Body** and confirm **Type = Dynamic**. Leave **Fixed Rotation** enabled; the reaction never changes that setting.
...
4. Confirm knockback launches at the Hitbox-authored velocity (currently horizontal `5`, vertical `3`) and does not rotate the fixed-rotation enemy.
